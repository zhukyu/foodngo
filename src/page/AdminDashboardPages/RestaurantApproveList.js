...
const RestaurantApproveList = () => {
  ...
  const handleMultDelete = () => {
    if(selectedRowKeys.length !== 0){
      Modal.confirm({
        title: "Are you sure you want to delete these requests ?",
        okText: "Yes",
        okType: "danger",
        onOk: () => {
        const newData = data.filter((row) => !(selectedRowKeys.includes(row.key) && (row.status[0] === "rejected")));
        setData(newData);
        setSelectedRowKeys([]);
    }});
  };}
  ...
  return (
    <div className="orders_table_container">
      <div>
        <h4 className="orders_table_title">Approve Restaurants</h4>
      </div>
